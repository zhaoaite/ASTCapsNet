...
def build_arch(input, y, is_train=False):
    ...
    with slim.arg_scope([slim.conv2d], trainable=is_train, weights_initializer=initializer, biases_initializer=biasInitializer):
        with tf.variable_scope('Conv1_layer') as scope:
            output = slim.conv2d(input, num_outputs=256, kernel_size=[3, 3], stride=1, padding='VALID', scope=scope)
            assert output.get_shape() == [cfg.batch_size_per_gpu, 7, 9, 256]

        with tf.variable_scope('PrimaryCaps_layer') as scope:
            output = slim.conv2d(output, num_outputs=32*8, kernel_size=[2, 2], stride=2, padding='VALID', scope=scope, activation_fn=None)
            output = tf.reshape(output, [cfg.batch_size_per_gpu, -1, 1, 8])
            assert output.get_shape() == [cfg.batch_size_per_gpu, 192, 1, 8]

        with tf.variable_scope('DigitCaps_layer') as scope:
            u_hats = []
            input_groups = tf.split(axis=1, num_or_size_splits=192, value=output)
            for i in range(192):
                u_hat = slim.conv2d(input_groups[i], num_outputs=16*20, kernel_size=[1, 1], stride=1, padding='VALID', scope='DigitCaps_layer_w_'+str(i), activation_fn=None)
                u_hat = tf.reshape(u_hat, [cfg.batch_size_per_gpu, 1, 20, 16])
                u_hats.append(u_hat)

            output = tf.concat(u_hats, axis=1)
            assert output.get_shape() == [cfg.batch_size_per_gpu, 192, 20, 16]

            b_ijs = tf.constant(np.zeros([192, 20], dtype=np.float32))
            v_js = []
            for r_iter in range(cfg.iter_routing):
                with tf.variable_scope('iter_'+str(r_iter)):
                    ...
                    for i in range(20):
                        c_ij = tf.reshape(tf.tile(c_ij_groups[i], [1, 16]), [192, 1, 16, 1])
                        ...
                        b_ij_groups[i] = b_ij_groups[i]+tf.reduce_sum(tf.matmul(tf.reshape(input_groups[i], [cfg.batch_size_per_gpu, 192, 16]), tf.reshape(v_j, [cfg.batch_size, 16, 1])), axis=0)
            ...
            if is_train:
                masked_v = tf.matmul(output, tf.reshape(y, [-1, 20, 1]), transpose_a=True)
                ...
                with tf.variable_scope('Decoder'):
                    ...
                    output = slim.fully_connected(output, 63, trainable=is_train, activation_fn=tf.sigmoid)
